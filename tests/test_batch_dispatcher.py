...
def test_dispatch_processing_alignment(tmp_path) -> None:
    ...
    @ai-ut-mock: patch 网络获取层 _fetch_properties_batch_network 和 _fetch_single_network。
    ...
    mock_batch_properties = {
        "PropertyTable": {
            "Properties": [
                {
                    "CID": 2244,
                    "InChIKey": "CID_KEY_2244",
                    "IUPACName": "Aspirin",
                    "CanonicalSMILES": "SMILES_2244",
    ...
    with patch("lib.batch_dispatcher._fetch_properties_batch_network", return_value=mock_batch_properties) as mock_batch, \
...
def test_dispatch_processing_interrupt_recovery(tmp_path) -> None:
    ...
    with patch("lib.batch_dispatcher._fetch_properties_batch_network", return_value=mock_batch_properties), \
...
def test_fetch_properties_batch_network() -> None:
    """
    @ai-ut-matrix: 测试 _fetch_properties_batch_network 批量获取属性的网络接口。
    @ai-ut-mock: 使用 MagicMock 模拟 urllib.request.urlopen 及其返回值。
    @ai-ut-assert: 1. 验证接口调用成功并正确解析返回数据。
                   2. 严格验证批量发送的 items 在 POST 请求体中是由逗号（编码后为 %2C）分隔。
    """
    import json
    mock_response = {
        "PropertyTable": {
    ...
    with patch("urllib.request.urlopen") as mock_urlopen:
        ...
        res = _fetch_properties_batch_network(
            "inchikey",
        ...
        # 深度契约断言：验证 payload 是否严格按照逗号分割并编码发送
        args, _ = mock_urlopen.call_args
        req = args[0]
        assert req.data == b"inchikey=CSCPPACGZOOCGX-UHFFFAOYSA-N%2CAICJARHUOUQZSH-UHFFFAOYSA-N"
...
def test_dispatch_processing_inchikey_batch(tmp_path) -> None:
    ...
    with patch("lib.batch_dispatcher._fetch_properties_batch_network", return_value=mock_batch_response) as mock_fetch:
        stats = dispatch_processing(
            sheets_results=sheets_results,
...
def test_dispatch_processing_inchikey_fallback(tmp_path) -> None:
    """
    测试当批量 InChIKey 查询发生异常时，是否能智能无感地自动降级为串行逐个属性查询。
    """
    cache_file = tmp_path / "cache.json"
    cache_manager = CacheManager(str(cache_file))
    ...
    # 模拟第一次批量查询报错，第二次单条查询成功
    call_cnt = 0
    def side_effect(input_type, items, properties):
        nonlocal call_cnt
        call_cnt += 1
        if call_cnt == 1:
    ...
    with patch("lib.batch_dispatcher._fetch_properties_batch_network", side_effect=side_effect) as mock_fetch:
